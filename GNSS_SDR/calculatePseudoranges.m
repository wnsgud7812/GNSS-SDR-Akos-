...
if(length(channelList)<1)
    disp('calculation failed!! The channelList is empty....');
    return;
end
for chanNr = (channelList)
    %--- Compute the travel times ----------------------------------------- 
    travelTime(chanNr) = trackResults(chanNr).absoluteSample(msOfTheSignal(chanNr)) / (2*samplesPerCode);
end
