...
% Generate 32 C/A codes to reduce calculation time 
for i = 1:1:32
    caCodesTable(i,:) = generateCAcode(i); 
end
...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        fseek(fid,settings.skipNumberOfBytes + 2*channel(channelNr).codePhase-2, 'bof');


        % Get a vector with the C/A code sampled 1x/chip
        caCode = caCodesTable(channel(channelNr).PRN,:);
...
%% Read next block of data ------------------------------------------------            
            ...
            % interation 
            [rawSignal, samplesRead] = fread(fid,2*blksize, settings.dataType);
            rawSignal = rawSignal';  %transpose vector
            
             ii=(rawSignal(1:2:2*blksize)); % in-phase signal
            qq= (rawSignal(2:2:2*blksize)); % quadrature signal
            complexSignal = ((ii)+1j*(qq));
...
%% Generate the carrier frequency to mix the signal to baseband -----------
            ...
            carrCos = real(exp(1j*trigarg(1:blksize)));
            % carrSin = sin(trigarg(1:blksize));
%% Generate the six standard accumulated values ---------------------------
            % First mix to baseband
            qBasebandSignal =(carrCos .* imag(complexSignal));
            iBasebandSignal = (carrCos.* real(complexSignal));
