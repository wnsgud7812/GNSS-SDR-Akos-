...
for channelNr = activeChnList
    if(subFrameStart(channelNr) + (1500 * 20) -1>length(trackResults(channelNr).I_P))
        disp('the subframe is too long. so this channel is been skipped');
        activeChnList = activeChnList(find(activeChnList~=channelNr));
        continue;
    end
...
for currMeasNr = 1:fix((settings.msToProcess - max(subFrameStart)) / settings.navSolPeriod) % 1:62
    ...
    navSolutions.channel.az(:, currMeasNr) = ...
                                         NaN(settings.numberOfChannels, 1);
   activeChnList = reshape(activeChnList,[1,(length(activeChnList))]);
