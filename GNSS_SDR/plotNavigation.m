...
if (~isempty(navSolutions))
    ...
    if isnan(settings.truePosition.E) || isnan(settings.truePosition.N) ...
       ...
       % meanLongitude = dms2mat(degrees2dms(mean(navSolutions.longitude(~isnan(navSolutions.longitude)))), -5);
        %meanLatitude  = dms2mat(degrees2dms(mean(navSolutions.latitude(~isnan(navSolutions.latitude)))), -5);
        meanLongitude = degrees2dms(mean(navSolutions.longitude(~isnan(navSolutions.longitude))));
        meanLatitude  = degrees2dms(mean(navSolutions.latitude(~isnan(navSolutions.latitude))));
